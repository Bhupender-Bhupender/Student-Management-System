...
app.config['MYSQL_PASSWORD'] = '1490'
app.config['MYSQL_DB'] = 'new'

# Intialization of MySQL
mysql = MySQL(app)
...
# Defining User class to represent users
...
def edit(id):
    if request.method == "POST":
        ...
        # Updating student record in the database
        cur = mysql.connection.cursor()
        cur.execute("""
            UPDATE student 
            SET rollno = %s, sname = %s, sem = %s, gender = %s, branch = %s, email = %s, number = %s, address = %s 
            WHERE id = %s
    ...
    # Fetching department data for dropdown
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM department")
    dept = cur.fetchall()
    cur.close()
    
    # Fetching student record to be edited
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM student WHERE id = %s", (id,))
    posts = cur.fetchone()
    cur.close()
